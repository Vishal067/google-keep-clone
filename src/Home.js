    ...
    if(state.pinned_id){
      ...
      <div className="pinned">
        ...
        <p key={index} className="list-item" id="li">
          <span className="span1">{item.title} <button className="pin-button" onClick={e=>removePin(item.id)}><img className="pin" alt='pin' src="./push-pin.png"/></button></span> 
...
const Home = ({state, styles, ...actions}) => {
    return(
        ...
        <div className="popup" style={state.showPopUp ? styles.inputStyle : styles.inputStyle1}>
            <p className="text" >
              <span className="edit-title"><input value={state.edited_note.title} onChange={(e)=>actions.handleChangeNote(e.target.value,"title", "edited_note")}/> <button onClick={e=>actions.pinNote(state.popUp_id)}><img className="pin" alt='pin' src="./push-pin.png"/></button></span>
        ...
        <ul>
          <Masonry
            ...
            {(!state.search ? state.notes_list : state.search_list).filter(n=>{ return ((n.id !== state.pinned_id) && (n.id !== state.popUp_id)) }).map((item,index)=>
              <li onDoubleClick={e => actions.showNote(item.id)} key={index} className="list-item" >
